...
void main() {
  ...
  // Desaturate before lighting, so the specular highlight and the grid
  // overlay keep their own colour on top of a monochrome plate.
  if (uGrayscale > 0.0) {
    float lum = dot(base, vec3(0.2126, 0.7152, 0.0722));
    base = mix(base, vec3(lum), clamp(uGrayscale, 0.0, 1.0));
  }
...
/**
 * ElasticMesh — ReactBits
 *
 * A textured plane simulated as a spring-coupled node grid. The pointer pulls
 * nearby nodes toward it and pushes them out in Z; neighbour coupling spreads
 * the deformation and the whole sheet oscillates back to rest. Normals are
 * recomputed per frame so the lighting bends with the surface.
 *
 * `backgroundColor` flattens the source image onto a solid fill first, so a
 * transparent or white-matted PNG lands on the site palette instead of showing
 * whatever RGB happens to sit under its alpha channel.
 */
const ElasticMesh = ({
  image = '',
  backgroundColor = '',
  ...
  useEffect(() => {
    ...
    if (image) {
      ...
      img.onload = () => {
        let source = img
        if (backgroundColor) {
          // Flatten onto a solid fill so transparent/white-matted PNGs sit on
          // the palette rather than on whatever is under their alpha.
    ...
    function frame(now) {
      ...
      program.uniforms.uGrayscale.value = p.grayscale
